# 🚀 WebSSH —— 像开网页一样管理服务器

---

## 这个工具是干什么的？🤔

你有很多台**远程电脑（服务器）** 要管？

- 以前：装各种软件、记一堆 IP、开好多窗口、手忙脚乱 😫
- 现在：**打开浏览器**，所有服务器列在网页上，**点一下就连上了**！

---

## 为什么要用 WebSSH？（5 大优点）🌟

### 1️⃣ 完全免费，不要钱 💰

| 工具 | 价格 |
|------|------|
| **WebSSH** | **0 元** ❤️ |
| Termius | 免费版有功能限制，高级版要收费 💰 |
| Royal TS | 要买许可证 💰 |
| MobaXterm | 免费版有功能限制 |

### 2️⃣ 网页版，不用安装 🌐

不用下载安装包！Windows、Mac、Linux、iPad 都能用，有浏览器就行。还可以**安装到桌面**像原生 App 一样使用。

### 3️⃣ 安全，银行级加密 🔒

- 密码用 **AES-256** 加密（美国军方也在用）
- 主密码保护（输入错误一万次都解不开）
- 备份文件有 **SHA-256 指纹**，别人改了你也知道

### 4️⃣ 功能比别的工具多 🛠️

- ✅ SSH / Telnet / Serial 串口 连接
- ✅ 网页终端（支持 **Ctrl+F 搜索**、字体/光标自定义）
- ✅ SFTP 文件管理（远程浏览、上传、下载、内联编辑）
- ✅ SSH 隧道/端口转发（本地、远程、动态三种）
- ✅ Docker 容器管理（查看/启动/停止容器）🐳
- ✅ **宏录制 / 自动化**（录制→回放→批量执行→定时任务）
- ✅ 命令片段（收藏常用命令，一键发送到终端）
- ✅ **服务器分组管理**（拖拽分组、置顶收藏、右键操作）
- ✅ 加密备份（换电脑一键恢复）+ **自动云同步（WebDAV）**
- ✅ 多标签终端 + **会话布局持久化**（刷新不丢失）
- ✅ 中英文切换 + 4 套配色主题 + 终端独立设置

### 5️⃣ 操作简单，点鼠标就行 🖱️

填好服务器地址、用户名、密码，点"连接"就完事了。

---

## 📊 和同类工具比一比

| 功能 | 🏆 **WebSSH** | Termius | Royal TS | MobaXterm | PuTTY |
|------|:-----------:|:-------:|:--------:|:---------:|:-----:|
| 免费 | ✅ **完全免费** | ❌ 高级版收费 | ❌ 收费 | ❌ 高级版收费 | ✅ |
| 网页版 | ✅ **是** | ❌ 要安装 | ❌ 要安装 | ❌ 要安装 | ❌ 要安装 |
| 加密存储密码 | ✅ **AES-256** | ✅ | ❌ | ❌ | ❌ |
| 主密码保护 | ✅ **有** | ✅ | ❌ | ✅ | ❌ |
| 宏录制/自动化 | ✅ **录制+批量+定时** | ❌ | ✅ | ❌ | ❌ |
| 串口协议 | ✅ **COM口支持** | ✅ | ✅ | ✅ | ❌ |
| SFTP 文件编辑 | ✅ **内联编辑** | ❌ | ✅ | ❌ | ❌ |
| 分组拖拽/置顶 | ✅ **有** | ✅ | ✅ | ❌ | ❌ |
| 会话布局持久化 | ✅ **刷新不丢失** | ✅ | ✅ | ❌ | ❌ |
| 终端内容搜索 | ✅ **Ctrl+F** | ❌ | ❌ | ✅ | ❌ |
| 备份加密 | ✅ **有** | ✅ | ❌ | ❌ | ❌ |
| 自动备份+云同步 | ✅ **每天/每周+WebDAV** | ✅ | ❌ | ✅ | ❌ |
| 命令片段 | ✅ **有** | ✅ | ✅ | ❌ | ❌ |
| SSH 隧道 | ✅ **三种** | ✅ | ✅ | ✅ | ✅ |
| Docker 管理 | ✅ **有！** 🐳 | ❌ | ❌ | ❌ | ❌ |
| 多套主题 | ✅ **4 种** | ✅ | ❌ | ✅ | ❌ |
| 终端独立设置 | ✅ **字体/光标/闪烁** | ✅ | ✅ | ✅ | ❌ |
| 中英文切换 | ✅ **一键切换** | ❌ | ❌ | ❌ | ❌ |
| 手机触控 | ✅ **触控键+缩放** | ✅ | ❌ | ❌ | ❌ |
| 多标签终端 | ✅ **独立标签** | ✅ | ✅ | ✅ | ❌ |
| PWA 安装到桌面 | ✅ **支持** | ❌ | ❌ | ❌ | ❌ |

---

## 🚀 安装部署

### 方法一：一键脚本（推荐）

```bash
curl -fsSL https://raw.githubusercontent.com/guoxpeng/webssh/main/deploy.sh | bash
```

一键完成：下载 → 安装 → 构建 → 启动。然后在浏览器打开 `http://你的IP:9627`

### 方法二：Docker

```bash
docker run -d --restart=unless-stopped --name webssh -p 9627:9627 \
  node:20-alpine sh -c "apk add git openssh && \
  git clone --depth=1 https://github.com/guoxpeng/webssh.git /app && \
  cd /app && npm install && npm run build && node server/index.mjs"
```

### 方法三：手动安装（一步步来）

```bash
git clone https://github.com/guoxpeng/webssh.git
cd webssh
npm install
npm run build
node server/index.mjs
```

打开 `http://localhost:9627`

### 方法四：开发模式（改代码用）

```bash
# 终端1：启动后端
node server/index.mjs

# 终端2：启动前端
npm run dev
```

前端 `http://localhost:5173`，后端 `http://localhost:9627`

---

## 📖 怎么使用？

### 第一步：设置主密码 🔐

第一次打开网页，设置一个**主密码**。用它来加密你保存的所有服务器密码。

> 就像手机锁屏密码，别人拿到你的电脑也看不到你的密码！

### 第二步：添加服务器 🔗

1. 填 **名称** — 起个名字（比如"我的博客服务器"）
2. 选 **分组** — 归到文件夹（比如"生产环境"、"测试环境"）
3. 填 **IP 地址** — 服务器地址（比如 192.168.1.100）
4. **端口** — 默认 22，不用改
5. **用户名** — 登录账号（比如 root）
6. **密码或密钥** — 选一个
7. 点 **连接** 就行！

### 第三步：管理服务器 📂

- 点 `+` **新建分组文件夹**
- **拖拽** 服务器到不同分组
- 右键分组：重命名 / 删除 / **一键连接全部**
- **置顶** 常用服务器
- 分组折叠状态自动记住

### 第四步：宏 / 自动化 🤖

- **录制**：点红点 → 输入命令 → 停止 → 保存
- **回放**：在宏面板点播放，自动逐条执行
- **批量**：勾选多台服务器 → 一键全部执行
- **定时**：设置每小时/每天/每周自动跑

### 第五步：开始用吧！🎉

连上后看到**黑底白字窗口**，就是服务器的命令行。

**快捷键：**
- `Ctrl+F` — 终端里搜索文字
- `Ctrl+P` — 宏面板
- `Ctrl+Tab` — 切换终端标签

---

## 功能速览 📋

### 🖥️ 终端操作
在网页里操作命令行。支持 Ctrl+F 搜索、调整字体、录制宏、多标签分屏。

### 📁 文件管理（SFTP）
浏览服务器文件。上传、下载、双击编辑保存，不用下载再上传。

### 🤖 宏 / 自动化
录制操作序列，一键回放或批量多台服务器执行。支持定时任务。

### 🚇 SSH 隧道
安全访问远程内网。支持本地转发、远程转发、动态转发三种模式。

### 🐳 Docker 管理
查看容器状态、启动、停止、看日志，网页上点鼠标就行。

### 📂 分组管理
拖拽分组、右键操作、置顶收藏、折叠状态持久化。

### 📝 命令片段
收藏常用命令，一键发送到终端，不用反复敲。

### 💾 备份管理
备份所有服务器配置，换电脑一键恢复。自动备份 + WebDAV 云同步。

### 📡 串口连接
通过 COM 口连交换机、路由器。波特率/数据位/停止位/校验位都能调。

### 🌏 国际化
点一下中英文切换。

### 🎨 主题换肤
浅色、深色、Dracula、Nord 四种配色，终端还能单独调字体和光标。

---

## 环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PORT` | `9627` | 服务端口 |
| `WS_PATH` | `/ws/ssh` | WebSocket 路径 |

---

## 技术栈

| 技术 | 用途 |
|------|------|
| Vue 3 | 前端框架 |
| xterm.js | 浏览器内终端模拟 |
| WebSocket | 实时双向通信 |
| ssh2 | Node.js SSH 客户端 |
| AES-256-GCM | 密码和备份加密 |
| Bulma | CSS 界面框架 |
| Vite | 构建工具 |
| Pinia | 状态管理 |
| Capacitor | 桌面/移动端原生包 |

---

> 💡 **小窍门**：点左边菜单的"服务器"开始，填好信息点连接就能用了！
