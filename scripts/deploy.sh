...
# 5) pkill by process name (both old and new paths)
pkill -f "node.*server/index.mjs" 2>/dev/null || true
pkill -f "node.*core/server/index.mjs" 2>/dev/null || true
sleep 2

# 6) Final check — if port still occupied, print owner and bail
...
rm -rf node_modules/.cache node_modules/.vite web/node_modules/.vite dist 2>/dev/null || true
...
ExecStart=$(command -v node) ${APP_DIR}/core/server/index.mjs
Restart=always
RestartSec=5
...
if [ "$USE_SYSTEMD" = true ]; then
  ...
  nohup env PORT="$PORT" node core/server/index.mjs > "$LOG_FILE" 2>&1 &
  local PID=$!
  echo "$PID" > "$PID_FILE"
  sleep 3
  # Verify PID file was written and process is alive
  if ! kill -0 "$PID" 2>/dev/null; then
    rm -f "$PID_FILE"
    err "Process failed to start. Check log: ${LOG_FILE}"
  fi
fi
